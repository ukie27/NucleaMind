...
from nucleamind.legacy.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("nucleamind.legacy.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("nucleamind.legacy.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("nucleamind.legacy.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
