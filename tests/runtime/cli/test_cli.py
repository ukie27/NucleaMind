...
def _perms(tmp_path: Path, *args: str) -> int:
    return app(["permissions", "--instance-dir", str(tmp_path), *args])


def test_permissions_help_and_empty_listing(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert _perms(tmp_path) == 0
    assert "grant" in capsys.readouterr().out
    assert _perms(tmp_path, "list") == 0
    # 「文件不存在」与「文件里没有记录」对用户是同一件事：还没装过任何插件。
    assert "还没有任何授权记录" in capsys.readouterr().out


def test_granting_then_listing_shows_the_decision(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert _perms(tmp_path, "grant", "notes", "fs:read", "整理笔记") == 0
    capsys.readouterr()
    assert _perms(tmp_path, "list") == 0
    out = capsys.readouterr().out
    assert "notes" in out
    assert "已授予  fs:read" in out
    assert "整理笔记" in out


def test_revoking_and_forgetting(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _perms(tmp_path, "grant", "notes", "shell", "跑构建")
    assert _perms(tmp_path, "revoke", "notes", "shell") == 0
    capsys.readouterr()
    assert _perms(tmp_path, "list", "--json") == 0
    assert '"decision": "revoked"' in capsys.readouterr().out

    assert _perms(tmp_path, "forget", "notes") == 0
    # 没有记录可删是退让而不是失败——与 `nm init` 撞上既有配置时同一个退出码。
    assert _perms(tmp_path, "forget", "notes") == 3


def test_a_bad_permission_name_is_reported(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert _perms(tmp_path, "grant", "notes", "filesystem") == 2
    assert "nm:" in capsys.readouterr().err
    assert _perms(tmp_path, "grant", "notes") == 2
    assert _perms(tmp_path, "nope") == 2
