"""`image` 插件测试的公共闸门：断言它不碰真实网络。

职责：一条 autouse 夹具，把出站连接与名字解析拦掉。本插件的用例全部走
`httpx.MockTransport`——本夹具是「一个 socket 都没开」这件事的可执行断言。
不负责：限制文件写入（本插件**会**写盘，用例用 `tmp_path`）。

**为什么不整体拦掉 `socket.socket` 的构造**：Windows 上 asyncio 的 `ProactorEventLoop`
自身要用 `socket.socketpair()` 做 self-pipe。拦构造会把事件循环一起拦掉，测试就只能
证明「起不来」。因此闸门放在**去哪儿**上：回环放行，其余一律失败并指名道姓。

这是全项目第七份同样判据的实现。刻意不共享：`R4` 禁止插件 import 宿主的测试树。
"""
