"""`anthropic` 插件测试的公共闸门：断言它不碰真实网络。

职责：一条 autouse 夹具，把出站连接与名字解析拦掉。本插件的用例全部走
`httpx.MockTransport`——本夹具是「一个 socket 都没开」这件事的可执行断言，而不是一句承诺。
...
这是全项目第三份同样判据的实现（另两份在 `tests/builtins/conftest.py` 与
`tests/integration/conftest.py`）。刻意不共享：`R4` 禁止插件 import 宿主的测试树，
而这三棵测试树各自可以独立运行。
"""
...
_MESSAGE = "anthropic 插件的测试试图访问真实网络"
