...
| `nucleamind-plugin-discord/` | `discord` | `CHANNEL:discord` —— Discord bot，配合 `nm serve` 常驻；出站附件真的上传（`D47`） | `D33` |
...
| `nucleamind-plugin-image/` | `image` | `TOOL:image.generate` —— 按文字描述生成图像，落进 workspace 并作为**出站附件**随本轮终帧发出（`D47`） | `D37` |
