...
- 更新时间：2026-08-18（`D52` 收口，**开放式插件权限模型定位**）
- 当前阶段：**阶段三 P1 能力插件化收口**（`D00`–`D52` 均已完成）。
  ...
  **`D52` 收口开放式插件权限模型定位**：安装并启用插件等同于信任其同进程 Python 代码；
  D26 的 manifest 声明、`permissions.json` 与资源门面继续负责意图审计、扩权记录和自愿
  约束，但不充当完整行为监控或安全沙箱。Kernel 不新增 `listen` / Agent 专属权限，
  监听型插件由 `plugins.enabled` / `plugins.disable` 与插件配置控制；强隔离留给可选宿主
  或部署环境。本轮只同步设计与文档，不改变生产行为或 SDK 表面。
...
- **M5 已全部交齐**，没有待迁移的模块。`D43`–`D45` 清掉的是「机制缺口」而不是新能力：
  ...
  有界续写；`D51` 再把摘要策略留给插件、Kernel 只保留压缩机制；`D52` 则把 D26 权限机制
  明确定位为受信任插件模型下的声明、审计与自愿门面约束。
...
- **`D20` 内建工具：`tools_fs`**（`builtins/tools_fs/` 九个模块约 1597 行 +
  ...
  - **TOCTOU 如实写进 docstring，不假装挡得住**：校验与随后的 `open()` 之间目标可被换成
    ...
    这是应用级守卫；更严格的隔离由可选独立宿主或部署环境承担，不进入极简 Kernel。
...
- **`D21` 内建工具：`tools_shell`**（`builtins/tools_shell/` 七个模块约 1100 行 +
  ...
  - **守住 cwd 不等于守住命令能碰到的文件**：一条 `cat /etc/shadow` 用绝对路径，与 cwd 无关。
    cwd 边界限制的是「命令默认在哪里落地」；更严格的隔离是不启用该能力，或使用独立宿主 /
    部署沙箱。这句如实写在 `paths.py` 里，不假装挡得住。同理**不移植 legacy 的
...
- **`D31` 遗留 Agent 路径切换与删除 ★**（删 `legacy/` 约 55600 行 + 新增
  ...
  - **两条如实记着的边界**（写进插件 docstring、README 与本文档，不留给用户发现）：
    ...
    「监听端口」**这一种（`net` 判的是出站），因此这个插件声明不出监听行为；默认只绑回环，
    绑非回环地址时没配 `api_key` 直接以 `CONFIG_INVALID` 拒绝启动。**① 已由 `D33`
    消除**（泵按 conversation 扇出）；② 的事实仍成立，但 `D52` 已决定这是受信任插件模型
    的刻意边界，不再作为待补权限空档。
...
- **`D33` Channel 泵扇出 + 官方 Discord 插件 ★**（两个可分别回退的 commit：
  ...
  - **两条如实记着的边界**：① **五种权限里没有「连接一个聊天平台」这一种**——`net` 判的是
    ...
    声明不出任何权限而它确实会连出去；`D52` 已明确这是受信任插件模型的刻意边界，不是
    待扩展的权限枚举；
...
- **`D52`**：开放式插件权限模型定位收口。**安装并启用插件即信任其代码**；D26 的声明、
  账本与 `PluginContext` 门面保留，但只承诺意图可审计、扩权可追踪和自愿使用门面时的应用级
  约束。明确不新增 `listen` / `net.listen`、不新增 Agent 专属权限系统，也不把未来子进程
  宿主写成 Kernel 的既定路线；监听型插件的闸门是启用状态与插件配置，更严格控制由可选
  安全插件、独立宿主或部署隔离承担。本轮零生产行为变化、零 SDK 表面变化。
...
上下文压缩策略已由 `D51` 以可选插件能力完成；权限模型定位已由 `D52` 收口。下面两类既有
事项仍然存在，**每一件都是先
要一个决定而不是先要工时**：
...
**监听权限不再是剩余事项。** `D52` 决定不新增 `listen` / `net.listen`：权限账本记录
manifest 主动声明的意图，不是同进程插件的完整行为清单；监听型插件由启用状态与配置控制。
