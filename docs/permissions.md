...
> 本文描述新 Kernel（`nm`）的权限模型。迁移期的 `nm legacy` 不适用。

## 这套机制不承诺什么

**应用级权限不是进程隔离。** 同进程的 Python 插件可以绕过这里描述的全部门面，直接
`import os`、`import httpx`、`subprocess.run(...)`。本机制的目标是：

- 让插件**声明**自己要什么，用户在安装/升级时看得见；
- 让越界意图在评审、代码审查与测试中**可见**；
- 让权限的变化**可审计**（`permissions.json` 进版本控制或备份即可 diff）。

它**不**防御恶意插件。真正的隔离依赖后续的子进程插件宿主（P2）——届时门面的方法签名
不变，底层换成 RPC，因此现在的接口形态是隔离方案的前置条件而不是障碍。
